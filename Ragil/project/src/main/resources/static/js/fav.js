...
song.forEach((e) => {
  for (let i = 1; i <= song.length; i++) {
    //   console.log(e.id);
    updateDataById(e.id, { id: i });
  }
});
n = 1;
const menuLagu = document.querySelector(".menulagu");

song.forEach((element) => {
  const itemLagu = document.createElement("li");
  const itemItteration = document.createElement("span");
  const posterImg = document.createElement("img");
  const titleSong = document.createElement("h5");
  const br = document.createElement("br");
  const artistSong = document.createElement("div");
  const playButton = document.createElement("i");

  console.log(element.idMusik);

  itemLagu.classList.add("itemlagu");
  artistSong.classList.add("subtittle");
  playButton.setAttribute("class", "bi playlistplay bi-play-circle");
  playButton.setAttribute("id", element.idMusik);
  if (n < 10) itemItteration.textContent = "0" + n;
  else itemItteration.textContent = n;
  artistSong.textContent = element.artist;
  posterImg.src = element.poster;
  titleSong.innerHTML =
    element.songName +
    "<br /><div class='subtittle'>" +
    element.artist +
    "</div>";

  // music.src = `audio/${n}.mp3`;
  playButton.addEventListener("click", (e) => {
    if (playButton.classList.contains("bi-play-circle")) {
  ...
  itemLagu.append(itemItteration);
  itemLagu.append(posterImg);
  itemLagu.append(titleSong);
  // itemLagu.append(br);
  // itemLagu.append(artistSong);
  itemLagu.append(playButton);
  menuLagu.append(itemLagu);
  n++;
});
